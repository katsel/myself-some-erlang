...
%% Mention that you're ready for a trade. When the other player also declares
%% being ready, the trade is done
ready(OwnPid) ->
...
%%% Ckient-to-client API (FSM to FSM functions)
...
%% Reply that the side is not ready to trade (i.e., not in 'wait' state)
not_yet(OtherPid) ->
...
%% Warn the other FSM that we cancelled the trade
...
%% The other player has sent this cancel event. Stop whatever we're doing and
%% shut down
...
%% This cancel event comes from the client. We must warn the other player that
%% we have a quitter
...
idle({ask_negotiate, OtherPid}, S=#state{}) ->  % asynchronous
...
idle({negotiate, OtherPid}, From, S=#state{}) ->  % syncronous
...
%% Other side retracting an offer
negotiate({undo_offer, Item}, S=#state{otheritems=OtherItems}) ->
   notice(S, "other player cancelling offer on ~p", [Item]),
   {next_state, negotiate, S#state{otheritems=remove(Item, OtherItems)}};
%%
negotiate(are_you_ready, S=#state{other=OtherPid}) ->
   io:format("other player ready to trade.~n"),
   notice(S,
...
%% Once decision is made, do synchronous call to avoid any sides modify offers
...
%% Send players a notice. This could be messages to their clients but for our
%% purposes, outputting to the shell is enough.
